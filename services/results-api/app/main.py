"""Results API application (docs/APP_SPEC.md §5).
...
# Every other SQLAlchemyError (e.g. IntegrityError, DataError, ProgrammingError) points to
# a bug or an unexpected state -> 500.
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        logger.info("Results API started (env=%s)", settings.app_env)
        try:
            yield
        finally:
            # Shutdown: close all pooled database connections.
            engine.dispose()
            logger.info("Results API stopped")
